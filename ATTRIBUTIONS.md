...
아래 목록은 제작 과정을 소스로 확인한 항목이다. DumpIt이 이용을 허락할 권리를 보유한 자체 제작 이미지는 [PolyForm Noncommercial License 1.0.0](LICENSE)을 따른다. 제3자 자료와 브랜드 자산은 [적용 범위](LICENSING.md)의 예외를 따른다.
...
| DumpIt 로고·문자 로고·favicon, 데스크톱 앱/트레이 ICO, 모바일 앱·스플래시 이미지 | 관리자가 로고를 AI로 자체 생성했다고 확인. 저장소의 리사이즈·변환 파일은 그 파생본 | 브랜드 자산으로서 코드 라이선스의 재사용 허락에서 제외. [적용 범위](LICENSING.md) 참조 |
...
현재 약관에는 원본 자료의 독립적인 재배포·재허락 등에 관한 제한이 있다. 앱에 포함한 이용과 원본 PNG를 공개 저장소에서 배포하는 이용은 각각 해당 조건을 확인한다. 이 자료는 DumpIt 자체 라이선스의 적용 대상에서 제외하며, 다운로드 시점의 개별 증빙 없이 별도의 재사용 권한을 부여하지 않는다.
