...
- 사이드바 안쪽 모서리에 세로 반복(repeat-y) 스트립, 상단바 아래 모서리에 가로 반복(repeat-x) 스트립.
- 구현: `Sidebar`/`Header`의 크롬 요소에 `app-sidebar`/`app-header` 클래스 훅 추가 후
  `[data-skin-chrome="sprout"] .app-sidebar::after { ... }` 식 스킨 스코프 CSS로 장식 부착
  (가로/세로 겸용이 되도록 정사각 타일로 설계, `pointer-events:none`).
- 장식 타일 4컨셉 × 라이트/다크 = 8장: 새싹=덩굴, 은하수=별무리, 원목=나뭇결 몰딩, 캔디=스프링클.
