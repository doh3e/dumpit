...
| 태양 | planet.sun | 800 | **애니**: 플레어 명멸 4프레임 |
| 블랙홀 | planet.blackhole | 1000 | **애니**: 강착원반 회전 4프레임, 신규 최고가 |

카탈로그 총 아이템 수 26 → 37.

## 애니메이션 렌더링 구조

- registry 엔트리 확장: `{ name, img }` → 애니 스프라이트는 `{ name, img, frames: 4, fps }` 추가.
