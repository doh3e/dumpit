...
export default function Sidebar({ onOpenSettings, onOpenHelp, tasks, focusRecommendation, isDrawerOpen, onCloseDrawer }) {
  ...
  return (
    <>
      ...
      <aside className="app-sidebar hidden lg:flex flex-col w-56 sticky top-20 self-start h-[calc(100vh-5rem)] overflow-y-auto scrollbar-none bg-chrome border-r border-chrome-line pt-6 pb-10 px-3 gap-1">
        {sidebarContent}
      </aside>
      ...
      {isDrawerOpen && (
        <div className="lg:hidden fixed inset-0 z-[55]" onClick={onCloseDrawer}>
          ...
          <aside
            className="app-sidebar absolute left-0 top-0 bottom-0 w-64 bg-chrome border-r border-chrome-line pt-6 pb-10 px-3 flex flex-col gap-1 overflow-y-auto"
